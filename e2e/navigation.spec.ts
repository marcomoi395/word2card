...
test.describe('Tab Navigation', () => {
    ...
    test('should show active button styling on current tab', async () => {
        ...
        hasActiveClass = await importButton.evaluate((el) => el.classList.contains('active-btn'))
    ...
    test('should change mascot background when switching tabs', async () => {
        ...
        let hasBgImport = await mascot.evaluate((el) => el.classList.contains('bg-import'))
        ...
        const hasBgNotion = await mascot.evaluate((el) => el.classList.contains('bg-notion'))
        expect(hasBgNotion).toBe(true)

        // Should not have bg-import anymore
        hasBgImport = await mascot.evaluate((el) => el.classList.contains('bg-import'))
        ...
        const hasBgSettings = await mascot.evaluate((el) => el.classList.contains('bg-settings'))
    ...
    test('should navigate through all three tabs sequentially', async () => {
        ...
        let notionActive = await window
            .locator('#section-notion')
            .evaluate((el) => el.classList.contains('active-section'))
        ...
        let settingsActive = await window
