...
import type { AppResponse, EditVocabularyPayload, VocabularyRecord } from '../../../shared/ipc'
...
export function registerCollectionHandlers(database: DatabaseRepositories): void {
    ...
    ipcMain.handle(IPC_CHANNELS.listVocabulary, async (): Promise<AppResponse<VocabularyRecord[]>> => {
        try { return success(service.list()) } catch (error) { return failure(error instanceof Error ? error.message : 'Failed to load collection') }
    })
