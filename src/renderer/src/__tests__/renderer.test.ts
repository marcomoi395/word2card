...
describe('Renderer UI', () => {
  ...
  describe('Settings Form', () => {
    ...
    it('shows default success message when saveSettings returns no message', async () => {
      const { promise, resolve: res } = Promise.withResolvers<void>()
      setTimeout(res, 0)
      await promise
    ...
    it('shows error alert when saveSettings returns error', async () => {
      ...
      expect(window.alert).toHaveBeenCalledWith(expect.stringContaining('Failed to save settings: Failed to save'))
    })
    ...
    it('shows error alert when saveSettings returns error without message', async () => {
      ...
      expect(window.alert).toHaveBeenCalledWith(expect.stringContaining('Failed to save settings: undefined'))
    })
    ...
    it('shows error alert when saveSettings throws error', async () => {
      ...
      expect(consoleSpy).toHaveBeenCalled()
      expect(window.alert).toHaveBeenCalledWith(expect.stringContaining('An error occurred while saving settings.'))
      consoleSpy.mockRestore()
    })
  })
  ...
  describe('File Import Flow', () => {
    ...
    it('does not call sendImport when both options are unchecked', async () => {
      ...
      const deckInput = form.elements.namedItem('deck') as HTMLInputElement
      const quizCheckbox = document.getElementById('chk-quiz-import') as HTMLInputElement
      const flashcardCheckbox = document.getElementById('chk-flashcard-import') as HTMLInputElement
      ...
      expect(window.api.sendImport).not.toHaveBeenCalled()
      expect(window.alert).toHaveBeenCalledWith(expect.stringContaining('Please select at least one import option'))
    })
    ...
    it('shows error alert when sendImport throws error', async () => {
      ...
      expect(consoleSpy).toHaveBeenCalled()
      expect(window.alert).toHaveBeenCalledWith(expect.stringContaining('An error occurred during import.'))
      consoleSpy.mockRestore()
    })
  })
  ...
  describe('Notion Sync Flow', () => {
    it('calls sendImport with NOTION_SYNC when form submitted', async () => {
      ...
      const deckInput = form.elements.namedItem('deck') as HTMLInputElement
      const flashcardCheckbox = document.getElementById('chk-flashcard-notion') as HTMLInputElement
    ...
    it('shows error alert when sendImport throws error', async () => {
      ...
      expect(consoleSpy).toHaveBeenCalled()
      expect(window.alert).toHaveBeenCalledWith(expect.stringContaining('An error occurred during sync.'))
      consoleSpy.mockRestore()
    })
  })
  ...
  describe('Tab Switching', () => {
    it('switches to notion tab', async () => {
      ...
      btn?.click()
      expect(document.getElementById('section-notion')?.classList.contains('active-section')).toBe(true)
    })
    ...
    it('switches to settings tab', async () => {
      ...
      btn?.removeAttribute('onclick')
      btn?.click()
      expect(document.getElementById('section-settings')?.classList.contains('active-section')).toBe(true)
    })
    ...
    it('switches to import tab', async () => {
      ...
      btn?.removeAttribute('onclick')
      btn?.click()
      expect(document.getElementById('section-import')?.classList.contains('active-section')).toBe(true)
    })
  })
